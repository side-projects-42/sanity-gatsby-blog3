...
module.exports = async function runCli(cliRoot) {
  ...
  const workDir = isInit ? process.cwd() : resolveRootDir(cwd); // Try to load .env files from the sanity studio directory
  ...
  cliRunner.runCommand(args.groupOrCommand, args, options).catch(err => {
    ...
    console.error(`\n${error.stack ? (0, _neatStack.default)(err) : error}`); // eslint-disable-next-line no-process-exit
